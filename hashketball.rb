...
#Find most_points_scored
def most_points_scored
  max_points = team_info.max_by {|stats_hash|stats_hash[:points]}
  team_info do |team_details|
    team_details[:players].each do |player, stats|
      return player if stats == max_points
    end
  end
...
##helper method for winning_team method to find the total scores of each team
def find_total_scores(team_details)
  total_scores = 0
  team_details[:players].values.each do |stats|
    total_scores += stats[:points]
  end
  return total_scores
end
...
def winning_team
  ...
  team_info do |team_details|
    team_scores[team_details[:team_name]] = find_total_scores(team_details)
  end
  team_scores.max_by {|team, score|score}.first
end

## returns player name with longest characters.
...
##returns true or false  if longest name steals a ton
def long_name_steals_a_ton?
  highest_steals_players = nil
  team_info do |team_details|
    highest_steals_players= team_details[:players].key(team_info.max_by {|stats_hash|stats_hash[:steals]})
  end
  highest_steals_players == player_with_longest_name
end
